        ...
        {scenes.map((scene) => {
          ...
          return (
            <div
              ...
              className={`w-full text-left rounded-xl px-3 py-2.5 transition cursor-pointer group ${active ? "bg-brand-faint ring-1 ring-brand-soft" : "hover:bg-gray-900/50"}`}
            >
              <div className="flex items-center justify-between gap-2 mb-1">
                <div className="flex items-center gap-2 min-w-0">
                  <span className={`text-[10px] font-mono ${active ? "text-brand" : "text-gray-500"}`}>S{scene.scene_number}</span>
              ...
              <div className="flex items-center gap-2 mt-2 text-[10px] text-gray-600">
                ...
                {generated > 0 && (
                  <span className={phase === "animate" ? "text-brand" : "text-gray-400"}>
                    · {generated} rendered
                  </span>
                )}
              </div>
...
function OutlineSidebar({ data }: ProjectSidebarProps) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4">
      <section className="rounded-2xl border border-gray-800 bg-gray-950/50 p-4 space-y-2.5">
